...
static int bdx_read_mac(struct bdx_priv *priv)
{
    ...
    macAddress[0] = READ_REG(priv, regUNC_MAC2_A);
    {
        u8 _mac[ETH_ALEN];
        for (i = 0; i < 3; i++) {
            _mac[i * 2 + 1] = macAddress[i];
            _mac[i * 2]     = macAddress[i] >> 8;
        }
        eth_hw_addr_set(priv->ndev, _mac);
    }
    RET(0);
}
...
bdx_get_coalesce(struct net_device *netdev, struct ethtool_coalesce *ecoal,
		 struct kernel_ethtool_coalesce *kcoal, struct netlink_ext_ack *extack)
{
    u32 rdintcm;
    u32 tdintcm;
    struct bdx_priv *priv = netdev_priv(netdev);
...
bdx_get_ringparam(struct net_device *netdev, struct ethtool_ringparam *ring,
		  struct kernel_ethtool_ringparam *kring, struct netlink_ext_ack *extack)
{
    struct bdx_priv *priv = netdev_priv(netdev);
...
static void bdx_ethtool_ops(struct net_device *netdev)
{
    ...
    static struct ethtool_ops bdx_ethtool_ops =
    {
        ...
        /* required since 5.7 when .set_coalesce is provided */
...
#endif
        /* get_settings/set_settings removed in kernel 4.20; use get_link_ksettings above */
